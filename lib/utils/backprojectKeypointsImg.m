function [keypointsAll,bboxAll] = backprojectKeypointsImg(expidx,firstidx,nImgs,bSave)

fprintf('backprojectKeypointsImg()\n');
...
for i = firstidx:lastidx
    ...
    try 
        ...
        for j = 1:length(p.pidxs)
            ...
            x = mean(det(:,[1 3]),2);
            y = mean(det(:,[2 4]),2);
            keypointsAll(i).det{jidx+1,:} = [[x y] det(:,5)];
            [val,idx] = max(det(:,5));
            points(jidx+1,:) = [x(idx) y(idx)];
        end
