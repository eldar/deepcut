function dist = getNormGTJointDist_nTop(keypointsAll,gt_annolist,jidx,bUseHeadSize,bHeadSizeFromRect,jidxGT,nTopDet)
    ...
    if (isfield(rect, 'annopoints'))
        ...
        if (~isempty(p))
            ...
            if ~iscell(det)
                ...
                nTop = min(size(keypointsAll(imgidx).det{jidx+1}(:,1:2)),nTopDet);
                points_det = keypointsAll(imgidx).det{jidx+1}(1:nTop,1:2);
