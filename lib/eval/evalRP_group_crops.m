function evalRP_group_crops(expidxs,bUseHeadSize,bSave)
...
%         end
    ...
    elseif (bp && ~roi)
        if (isfield(p,'img_2_people_only') && p.img_2_people_only)
            if (expidx == 357)
                keypointsAll = inf2keypointsImgBPcell(expidx,parts,annolist_gt);
                assert(length(annolist_gt) == length(keypointsAll));
            else
                keypointsAll = inf2keypointsImgBPcell(expidx,parts,annolist_gt,imgidxs_gt);
            end
        else
            keypointsAll = inf2keypointsImgBPcellMulti(expidx,parts,annolist_gt,imgidxs_gt);
            assert(length(annolist_gt) == length(keypointsAll));
        end
        
    elseif (roi)
        keypointsAll = inf2keypointsImgBP_roi(expidx,parts,annolist_gt,boxesNeck,imgidxs_gt,marg_scores);
    else
        keypointsAll = det2keypointsImg(expidx,parts,annolist_gt,p.top_det,false,false);
        save([p.evalTest '.mat'],'keypointsAll');
        assert(length(annolist_gt) == length(keypointsAll));
    end
    
    rect_ignore = cell(length(annolist_gt),1);
        
    if (isfield(p,'img_2_people_only') && p.img_2_people_only)
%         rect_ignore = rect_ignore(nrects == 2);
%         rectidxs_gt = rectidxs_gt(nrects == 2);
        if (~roi && ~(expidx == 377))
...
%                 load(fboxes,'bboxAll');
                ...
                for imgidx = 1:length(keypointsAll)
                    ...
                    rect = annolist_gt(imgidx).annorect;
                    refDist = p.refHeight/8;
                    keep = nms_dist(keypointsAll(imgidx).det{9},nms_thresh_det,refDist);
                    keep = keep(1:min(length(keep),nTrials));
%                     keep = 1:size(keypointsAll(imgidx).det{9},1);
...
%                     end
    ...
    if (bp && ~roi)
        if (isfield(p,'img_2_people_only') && p.img_2_people_only)
            [distAll,scoresAll,pointsAll] = assignGTinference(keypointsAll,annolist_gt,p.pidxs,parts,0.5,marg_scores,2);
        else
            [distAll,scoresAll,pointsAll] = assignGTinferenceMulti(keypointsAll,annolist_gt,p.pidxs,parts,0.5,marg_scores,nTrials);
        end
