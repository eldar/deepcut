function [tp_score,fp_score,totalPos,tp_points,fp_points,tp_idxs,fp_idxs,fp_dist,missing_recall] = computeTFP_spatial(dist,score,points,rectidxs_gt,rect_ignore,thresh_gt)
...
%     if (isempty(score{imgidx}))
%         continue;
%     end
    
    assert(size(dist{imgidx},1) == size(score{imgidx},1));
    d = sqrt((dist{imgidx,1}.^2) + (dist{imgidx,2}.^2));
    [minDist,minIdx] = min(d,[],2);
    
    match_gt = zeros(size(dist{imgidx,1},1),1);
    minDist = inf(size(dist{imgidx,1},1),2);
    for ridx = 1:length(minIdx)
        match_gt(ridx) = dist{imgidx,1}(ridx,minIdx(ridx)) <= thresh_gt & dist{imgidx,2}(ridx,minIdx(ridx)) <= thresh_gt;
        minDist(ridx,:) = [dist{imgidx,1}(ridx,minIdx(ridx)) dist{imgidx,2}(ridx,minIdx(ridx))];
    end
...
%             fprintf('not a member!\n');
    ...
    idxs_absent = find(sum(isinf(dist{imgidx}),1) == size(dist{imgidx},1));
